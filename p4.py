...
with open("Project 4 -Amazon Music clustering/feature_col.pkl", 'rb') as file:
    feature_col = joblib.load(file)

st.set_page_config(page_title="🎵 Amazon Music Clustering Dashboard", layout="wide")
@st.cache_data
def load_data():
    df = pd.read_csv("Project 4 -Amazon Music clustering/amazon_music_clusters_k3.csv")
    summary = pd.read_csv("Project 4 -Amazon Music clustering/amazon_music_final_summary.csv")
    return df, summary
